...
def range_embedding(height, width, embedding_dim):
        # Calculate position indices for each pixel
        x_coords = torch.arange(width).repeat(height, 1).float()  # Horizontal (wraps around)
        y_coords = torch.arange(height).view(height, 1).repeat(1, width).float()  # Vertical (regular)
        
        # Normalize the coordinates for sinusoidal encoding
        x_coords = 2 * math.pi * x_coords / width  # Wrap-around in the horizontal direction
        y_coords = 2 * math.pi * y_coords / height  # Regular in the vertical direction
        
        # Create embedding
        embedding = torch.zeros((1, height * width, embedding_dim))
        
        # Horizontal encoding with wrap-around
        for i in range(embedding_dim // 2):
            embedding[0, :, 2 * i] = torch.sin((i + 1) * x_coords).view(-1)  # Horizontal sine
            embedding[0, :, 2 * i + 1] = torch.cos((i + 1) * x_coords).view(-1)  # Horizontal cosine
            
        # Vertical encoding (regular)
        for i in range(embedding_dim // 2, embedding_dim):
            embedding[0, :, 2 * (i - embedding_dim // 2)] += torch.sin((i + 1) * y_coords).view(-1)  # Vertical sine
            embedding[0, :, 2 * (i - embedding_dim // 2) + 1] += torch.cos((i + 1) * y_coords).view(-1)  # Vertical cosine
...
class TransformerClassifier(nn.Module):
    def __init__(self,
        ...
        # elif positional_embedding == 'range':
        #     self.positional_emb = range_embedding(height, width, embedding_dim)
        else:
