...
class AttentionalGNN(nn.Module):
    def __init__(self, feature_dim: int, num_layer: int, num_heads: int):
        super().__init__()
        self.layers = nn.ModuleList([
            AttentionalPropagation(feature_dim, num_heads)
            for _ in num_layer]) # self.config['descriptor_dim']

    def forward(self, feature, src, k, L):
        i = 0
...
class SelfAttentionGNN(nn.Module):
    def __init__(self, d_model=256, nhead=4, dim_feedforward=1024, activation='relu', batch_first=False, dropout=0.1, layer_num=3):
        super().__init__()
        ...
        self.gnn = AttentionalGNN(
            feature_dim=self.d_model, num_layer=self.layer_num, num_heads=self.nhead)

        self.final_proj = nn.Conv1d(self.d_model, self.d_model, kernel_size=1, bias=True)
                
    def forward(self, data):
        # data -> # [6, 256, 900] [batch, dim, width]
