...
from utils.data_loaders.nclt.nclt_dataset import NCLTTupleDataset
...
class NCLTSparseTupleDataset(NCLTTupleDataset):
    r"""
    ...
    def __init__(self,
        ...
        NCLTTupleDataset.__init__(self, phase, random_rotation, random_occlusion, random_scale, config)

        self.logger.info("NCLTSparseTupleDataset")
    ...
    def __getitem__(self, idx):
        ...
        query_tensor = self.get_pointcloud_sparse_tensor(drive_id, query_id)
        ...
        if not self.quadruplet:
            ...
            other_negative_tensor = self.get_pointcloud_sparse_tensor(drive_id, other_negative_id)
...
def test_datasets():
    ...
    kitti_sparse_tuple_dataset = NCLTSparseTupleDataset(phase='train', config=config)
    ...
    kitti_point_sparse_tuple_dataset = NCLTPointSparseTupleDataset(phase='train', config=config)
    print("KittiPointSparseTupleDataset 테스트 - 첫 번째 샘플:")
    query_tensor = kitti_point_sparse_tuple_dataset[0]['query']
    positives = kitti_point_sparse_tuple_dataset[0]['positives']
    negatives = kitti_point_sparse_tuple_dataset[0]['negatives']
    other_negative = kitti_point_sparse_tuple_dataset[0]['other_neg']
    meta_info = kitti_point_sparse_tuple_dataset[0]['meta_info']
