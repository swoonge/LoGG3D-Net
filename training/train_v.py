    ...
    for epoch in range(starting_epoch, cfg.max_epoch):
        ...
        for i, batch in enumerate(progress_bar):
            if cfg.train_pipeline == 'LOGG3D':
                ...
                current_batch = torch.unsqueeze(current_batch, dim=1) # [6,1,64,900]
            ...
            if (i % cfg.loss_log_step) == (cfg.loss_log_step - 1):
                ...
                if (dist.rank() % dist.size() == 0):
                    lr = scheduler.get_last_lr()
                    tqdm.write(f"Batch: {i}, Avg Loss: {avg_loss:.4f}, LR: {lr[0]:.6f}")
                    # tqdm.set_postfix(avg_loss=f"{avg_loss:.4f}", lr=f"{lr[0]:.6f}")
                    ...
                    writer.add_scalar('learning rate', lr[0], epoch * len(train_loader) + i)
        ...
        #         writer.add_scalar('val loss', val_loss/loss_num, epoch)
