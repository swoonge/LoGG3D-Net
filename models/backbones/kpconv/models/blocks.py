...
from models.backbones.kpconv.kernels.kernel_points import load_kernels
...
class KPConv(nn.Module):
    ...
    def __init__(self, kernel_size, p_dim, in_channels, out_channels, KP_extent, radius,
                 ...
                 deformable=False, modulated=False, radius_scaling=None):
        """
        ...
        self.radius_scaling = radius_scaling  # 추가된 부분
    ...
    def forward(self, q_pts, s_pts, neighb_inds, x):
        ...
        if self.radius_scaling is not None:
            # r 값은 q_pts의 첫 번째 열에서 추출합니다.
            # print(self.KP_extent)
            r_values = q_pts[:, 0]  # r 값 추출 (첫 번째 인덱스)
            dynamic_extent = self.KP_extent * self.radius_scaling(r_values).view(-1, 1, 1)  # 각 포인트별로 커널 반경 조정
            dynamic_extent = torch.clamp(dynamic_extent, max=self.KP_extent)  # 최댓값은 KP_extent로 설정

        else:
            dynamic_extent = self.KP_extent
        ...
        if self.deformable:
            ...
            offsets = unscaled_offsets * dynamic_extent
        ...
        # 커널 포인트의 영향 반경을 dynamic_extent로 변경
        sq_distances = torch.sum(differences ** 2, dim=3) / (2 * dynamic_extent ** 2)
        ...
        if self.deformable:
            ...
            in_range = torch.any(sq_distances < dynamic_extent ** 2, dim=2).type(torch.int32)
        ...
        if self.KP_influence == 'constant':
            ...
            all_weights = torch.clamp(1 - torch.sqrt(sq_distances) / dynamic_extent, min=0.0)
            all_weights = torch.transpose(all_weights, 1, 2)

        elif self.KP_influence == 'gaussian':
            # Influence in gaussian of the distance.
            sigma = dynamic_extent * 0.3
...
def block_decider(block_name,
    ...
    if block_name == 'unary':
        ...
        return SimpleBlock(block_name, in_dim, out_dim, radius, radius_scaling, layer_ind, config)
        ...
        return ResnetBottleneckBlock(block_name, in_dim, out_dim, radius, radius_scaling, layer_ind, config)
...
class SimpleBlock(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, radius_scaling, layer_ind, config):
        """
